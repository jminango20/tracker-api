...
export class ContractValidators {
    ...
    // Validations for Transactions Orchestrator
    static validateTransactionRequest(request: any) {
        ...
        const processIdValidation = this.validateProcessId(request.processId);
        if (!processIdValidation.isValid) {
            return {
                isValid: false,
                error: 'Process ID: ' + processIdValidation.error
            };
        }

        const natureIdValidation = this.validateNatureId(request.natureId);
        if (!natureIdValidation.isValid) {
            return {
                isValid: false,
                error: 'Nature ID: ' + natureIdValidation.error
            };
        }

        const stageIdValidation = this.validateStageId(request.stageId);
        if (!stageIdValidation.isValid) {
            return {
                isValid: false,
                error: 'Stage ID: ' + stageIdValidation.error
            };
        }

        const channelValidation = this.validateContractName(request.channelName);
        if (!channelValidation.isValid) {
            return {
                isValid: false,
                error: 'Channel name: ' + channelValidation.error
            };
        }

        const assetIdsValidation = this.validateAssetIdArray(request.targetAssetIds);
        if (!assetIdsValidation.isValid) {
            return assetIdsValidation;
        }

        const operationDataValidation = this.validateOperationData(request.operationData);
        if (!operationDataValidation.isValid) {
            return operationDataValidation;
        }

        if (request.dataHash && typeof request.dataHash !== 'string') {
            return {
                isValid: false,
                error: 'dataHash deve ser uma string'
            };
        }

        if (request.dataHashes && !Array.isArray(request.dataHashes)) {
            return {
                isValid: false,
                error: 'dataHashes deve ser um array'
            };
        }
    ...
    static validateAssetIdArray(assetIds: any) {
        if (!Array.isArray(assetIds)) {
            return {
                isValid: false,
                error: 'targetAssetIds deve ser um array'
            };
        }

        // Validar cada asset ID
        for (let i = 0; i < assetIds.length; i++) {
            if (!assetIds[i] || typeof assetIds[i] !== 'string') {
                return {
                    isValid: false,
                    error: `Asset ID na posição ${i + 1} deve ser uma string não vazia`
                };
            }

            if (!/^[a-zA-Z0-9_-]+$/.test(assetIds[i])) {
                return {
                    isValid: false,
                    error: `Asset ID na posição ${i + 1} deve conter apenas letras, números, underscore e hífen`
                };
            }
        }
    ...
    static validateOperationData(operationData: any) {
        if (!operationData || typeof operationData !== 'object') {
            return {
                isValid: false,
                error: 'operationData é obrigatório e deve ser um objeto'
            };
        }

        if (operationData.initialAmount !== undefined) {
            if (typeof operationData.initialAmount !== 'number' || operationData.initialAmount < 0) {
                return {
                    isValid: false,
                    error: 'initialAmount deve ser um número não negativo'
                };
            }
        }

        if (operationData.newAmount !== undefined) {
            if (typeof operationData.newAmount !== 'number' || operationData.newAmount < 0) {
                return {
                    isValid: false,
                    error: 'newAmount deve ser um número não negativo'
                };
            }
        }

        const stringFields = ['initialLocation', 'targetOwner', 'externalId', 'newAssetId', 'newLocation'];
        for (const field of stringFields) {
            if (operationData[field] !== undefined && typeof operationData[field] !== 'string') {
                return {
                    isValid: false,
        ...
        if (operationData.targetOwner) {
            const addressValidation = this.validateAddress(operationData.targetOwner);
            if (!addressValidation.isValid) {
                return {
                    isValid: false,
                    error: 'targetOwner: ' + addressValidation.error
                };
            }
        }

        if (operationData.splitAmounts !== undefined) {
            if (!Array.isArray(operationData.splitAmounts)) {
                return {
                    isValid: false,
                    error: 'splitAmounts deve ser um array'
                };
            }

            if (operationData.splitAmounts.length > 20) {
                return {
                    isValid: false,
                    error: 'Máximo de 20 valores em splitAmounts'
                };
            }

            for (let i = 0; i < operationData.splitAmounts.length; i++) {
                if (typeof operationData.splitAmounts[i] !== 'number' || operationData.splitAmounts[i] <= 0) {
                    return {
                        isValid: false,
                        error: `splitAmount na posição ${i + 1} deve ser um número positivo`
                    };
                }
            }
        }
    ...
    static validateAssetId(assetId: any) {
        ...
        if (!/^[a-zA-Z0-9_-]+$/.test(assetId)) {
            return {
                isValid: false,
                error: 'ID do assetId deve conter apenas letras, números, underscore e hífen'
            };
        }
