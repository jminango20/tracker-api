...
export class AccessChannelManagerService extends BlockchainService {
  ...
  private async ensureContractAddress(): Promise<void> {
    ...
    if (!result.success || !result.data?.addressContract) {
      throw new Error(
        "AccessChannelManager não encontrado no AddressDiscovery"
      );
    }
  ...
  async createChannel(
    ...
    try {
      ...
      const errorInfo =
        contractError || this.handleBlockchainError(error, "criar canal");
  ...
  async activateChannel(
    ...
    try {
      ...
      const txHash = (await contract.write.activateChannel([
        channelHash,
      ])) as any;
      ...
      const errorInfo =
        contractError || this.handleBlockchainError(error, "ativar canal");
  ...
  async deactivateChannel(
    ...
    try {
      ...
      const txHash = (await contract.write.desactivateChannel([
        channelHash,
      ])) as any;
      ...
      const errorInfo =
        contractError || this.handleBlockchainError(error, "desativar canal");
  ...
  async addChannelMember(
    ...
    try {
      console.log(
        `Adicionando membro ${memberAddress} ao canal: ${channelName}`
      );
      ...
      const errorInfo =
        contractError || this.handleBlockchainError(error, "adicionar membro");
      return {
        success: false,
        error: errorInfo.message,
      };
  ...
  async removeChannelMember(
    ...
    try {
      ...
      const errorInfo =
        contractError || this.handleBlockchainError(error, "remover membro");
  ...
  async addChannelMembers(
    ...
    try {
      console.log(
        `Adicionando ${memberAddresses.length} membros ao canal: ${channelName}`
      );
      ...
      const errorInfo =
        contractError || this.handleBlockchainError(error, "adicionar membros");
  ...
  async removeChannelMembers(
    ...
    try {
      console.log(
        `Removendo ${memberAddresses.length} membros do canal: ${channelName}`
      );
      ...
      const errorInfo =
        contractError || this.handleBlockchainError(error, "remover membros");
  ...
  async isChannelMember(
    ...
    try {
      console.log(
        `Verificando se ${memberAddress} é membro do canal: ${channelName}`
      );
      ...
      return {
        ...
        data: {
          ...
          message: `${memberAddress} ${
            Boolean(isMember) ? "é" : "não é"
          } membro do canal '${channelName}'`,
        },
      };
    } catch (error) {
      const contractError = ContractErrorHandler.handleContractError(
      ...
      const errorInfo =
        contractError || this.handleBlockchainError(error, "verificar membro");
  ...
  async getChannelMembersPaginated(
    ...
    try {
      console.log(
        `Buscando membros do canal ${channelName} - página ${page}, tamanho ${pageSize}`
      );
      ...
      const errorInfo =
        contractError ||
        this.handleBlockchainError(error, "obter membros paginados");
  ...
  async getChannelInfo(channelName: string): Promise<ApiResponse<ChannelInfo>> {
    try {
      ...
      const errorInfo =
        contractError ||
        this.handleBlockchainError(error, "buscar informações do canal");
  ...
  async getChannelCount(): Promise<ApiResponse<any>> {
    try {
      ...
      const errorInfo =
        contractError ||
        this.handleBlockchainError(error, "obter total de canais");
  ...
  async getAllChannelsPaginated(
    ...
    try {
      ...
      const result = (await contract.read.getAllChannelsPaginated([
        page,
        pageSize,
      ])) as any;
      ...
      const errorInfo =
        contractError ||
        this.handleBlockchainError(error, "obter canais paginados");
  ...
  async getChannelMemberCount(channelName: string): Promise<ApiResponse<any>> {
    try {
      ...
      const memberCount = (await contract.read.getChannelMemberCount([
        channelHash,
      ])) as any;
      ...
      const errorInfo =
        contractError ||
        this.handleBlockchainError(error, "obter total de membros");
  ...
  async areChannelMembers(
    ...
    try {
      console.log(
        `Verificando ${memberAddresses.length} membros do canal: ${channelName}`
      );
      ...
      const results = (await contract.read.areChannelMembers([
        channelHash,
        addresses,
      ])) as any;
      ...
      console.log(
        `Verificação completa para ${memberAddresses.length} endereços`
      );
      ...
      const errorInfo =
        contractError ||
        this.handleBlockchainError(error, "verificar múltiplos membros");
