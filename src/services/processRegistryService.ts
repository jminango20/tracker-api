...
export class ProcessRegistryService extends BlockchainService {
  ...
  async createProcess(
    ...
    try {
      console.log(
        `Criando processo: ${processInput.processId} no canal: ${processInput.channelName}`
      );
      ...
      const errorInfo =
        contractError || this.handleBlockchainError(error, "criar processo");
  ...
  async setProcessStatus(
    ...
    try {
      ...
      console.log(
        `Alterando status do processo: ${processId} para ${newStatus}`
      );
      ...
      const errorInfo =
        contractError ||
        this.handleBlockchainError(error, "alterar status do processo");
  ...
  async inactivateProcess(
    ...
    try {
      console.log(
        `Inativando processo: ${processId} nature: ${natureId} stage: ${stageId}`
      );
      ...
      const errorInfo =
        contractError || this.handleBlockchainError(error, "inativar processo");
  ...
  async getProcess(
    ...
    try {
      console.log(
        `Buscando processo: ${processId} nature: ${natureId} stage: ${stageId} no canal: ${channelName}`
      );
      ...
      console.log(
        `Processo encontrado - status: ${process.status}, action: ${process.action}`
      );
  ...
  async validateProcessForSubmission(
    ...
    try {
      console.log(
        `Validando processo para submissão: ${processId} nature: ${natureId} stage: ${stageId}`
      );
      ...
      return {
        ...
        data: {
          ...
          message: Boolean(isValid)
            ? "Processo válido para submissão"
            : "Processo não válido para submissão",
        },
      };
    } catch (error) {
      const contractError = ContractErrorHandler.handleContractError(
      ...
      const errorInfo =
        contractError ||
        this.handleBlockchainError(error, "validar processo para submissão");
  ...
  private convertActionStringToNumber(actionString: string): number {
    const actionKey = actionString
      .toUpperCase()
      .replace(" ", "_") as keyof typeof ProcessAction;
