...
export class AddressDiscoveryService extends BlockchainService {
    ...
    async getAddress(contractName: string): Promise<ApiResponse<ContractData>> {
        try {
            ...
            const addressContract = await contract.read.getContractAddress([contractHash]) as any;
        ...
        try {
            ...
            return {
                ...
                data: {
                    contractName,
                    isRegistered,
                    message: `O contrato ${contractName} ${isRegistered ? 'está' : 'não está'} registrado`
                }
            };
    ...
    async updateAddress(contractName: string, newAddress: string, privateKey: string): Promise<ApiResponse<any>> {
        try {
            console.log(`Atualizando endereço de: ${contractName} para: ${newAddress}`);
